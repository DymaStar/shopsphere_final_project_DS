...
Файл містить supporting queries A1–A8.
...
- A7 → tableau_a7_customer_value_by_channel.csv
- A8 → tableau_a8_category_interactive.csv

A2 (Regional YoY Growth) використовується лише як аналітична
перевірка регіонального зростання і НЕ експортується окремим CSV.
Основний multi-line chart будується з результату Task 1.1.

ВИПРАВЛЕННЯ (07.2026, аудит):
- прибрано "хвіст" дубльованого коду в кінці запиту A7, який
  залишився після copy-paste і робив файл невиконуваним як єдиний
  скрипт;
- узгоджено назву експортованого файлу A7: раніше файл фактично
  зберігався як tableau_a2_customer_value_by_channel.csv, хоча за
  змістом це результат запиту A7, а не A2. Перед підключенням у
  Tableau перейменуй існуючий CSV на tableau_a7_customer_value_by_channel.csv
  (або онови посилання на джерело даних у книзі, якщо назва вже
  використовується у published worksheets).
*/
